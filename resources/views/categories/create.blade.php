    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white border-4 border-black shadow-neo-lg p-8 relative overflow-hidden">
                <!-- Decorative corner -->
                <div class="absolute top-0 right-0 w-16 h-16 bg-neo-yellow border-b-4 border-l-4 border-black -mr-8 -mt-8 transform rotate-45"></div>
                ...
                <form method="POST" action="{{ route('categories.store') }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name')" required autofocus autocomplete="name" placeholder="ENTER CATEGORY NAME..." />
                        ...
                        <div class="relative">
                            <select id="parent_id" name="parent_id" class="block mt-1 w-full px-4 py-3 bg-neo-white border-2 border-black text-black font-bold focus:outline-none focus:border-neo-blue focus:shadow-neo-sm transition-all duration-200 appearance-none">
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" {{ old('parent_id') == $category->id ? 'selected' : '' }}>{{ strtoupper($category->name) }}</option>
                                @endforeach
                            </select>
